<form method="POST" action="">
    ...
    <input type="number" id="price" name="price" value="0" required><br><br>  <!-- Value "0" 추가, 변수 워닝 관련 --> 

    <label for="membership">회원 등급:</label>
    <select id="membership" name="membership" value="일반" required>  <!-- Value "0", 변수 워닝 관련 추가 -->
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // 폼에서 입력된 데이터 받기
    $price = $_POST['price']; // 변수 undefined 워닝 발생한 학생들은 ==>isset()함수 참고 하기 바랍니다.(coupon 라인 참고) (솔루션))
    $membership = $_POST['membership']; // 변수 undefined 워닝 발생한 학생들은 ==>isset()함수 참고 하기 바랍니다.(coupon 라인 참고)
    $coupon = isset($_POST['coupon']) ? true : false;  #isset : 함수는 변수가 선언되어 있고, null이 아닌 경우에만 true를 반환
